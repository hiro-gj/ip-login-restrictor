...
*   **有効期限設定**: 指定した日時を過ぎるとページ制限を自動で無効化。空欄にすると無期限で制限を維持します。
...
### 5. スマートな下書きプレビュー（Draft Preview）📝
ログインしていない外部パートナー（ライターやクライアント）でも、許可されたIPアドレスからであれば、**下書き（Draft）やレビュー待ち（Pending）** 状態の記事を直接プレビュー可能です。
プレビュー中は画面下部にわかりやすい通知バーが表示されるため、公開状態と間違えることもありません。

### 6. その他のこだわり機能
*   **IPv4 / IPv6 完全対応**: 最新のIPv6環境にも対応。アクセス時にIPv6アドレス（もしあれば）を自動検出し、許可リストと照合します。IPv4/IPv6デュアルスタック環境でも、どちらかのIPが許可されていればスムーズにアクセスできます。
*   **HTMLメッセージカスタマイズ**: アクセス拒否画面のメッセージを自由に編集可能。`{ip}`, `{datetime}`, `{site_name}`, `{page_message}` 等のトークンが使用可能です。
*   **ロケール（タイムゾーン）対応**: 有効期限の判定や時刻表示は、WordPress の「タイムゾーン」設定に基づきます。
*   **ワンクリック追加**: 管理画面にアクセス中の自分のIP（IPv4/IPv6）を、ボタン一つでリストに追加できます。
*   **CIDR対応**: 個別のIPだけでなく、`192.168.1.0/24` や `2001:db8::/32` のような範囲指定もサポート。
